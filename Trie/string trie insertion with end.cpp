...
int dfs(int i, int p, int k) {
    if(k>1) return 0;
    if(i == s.length() && vis[p] && k==1) return 1;
    if(i>=s.length()) return 0;
    int x = s[i]-'a', ans=0, j;
    if(tr[p][x]) {
        if(dfs(i+1, tr[p][x], k)) return 1;
    }
    if(k) return 0;
    rep(j,0,3) {
        if(j == x) continue;
        if(tr[p][j]) if(dfs(i+1, tr[p][j], k+1)) return 1;
    }
    return 0;
}
...
// while passing string in recursion: memory limit exceeded so make a global string.
// OR make the string pass by reference using '&' as function parameter...............